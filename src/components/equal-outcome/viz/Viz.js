...
class Viz extends Component {
  ...
  createViz(data, config, target) {
    ...
    // X-AXIS
    const xScale = d3.scaleBand().domain(data.xScale).range([0, innerWidth])
    ...
    // Y-AXIS
    const yScale = d3.scaleLinear().domain(data.yScale).range([innerHeight, 0])
    ...
    if (config.drawBars) {
      svg
        ...
        .attr(
          "transform",
          `translate(${margin.left + xScale.bandwidth() * 0.1} ${margin.top})`
        )
        .selectAll(".bar")
        .data(data.content)
        .enter()
    ...
    if (config.drawMean) {
      ...
      const meanGroup = svg
        ...
        .attr(
          "transform",
          `translate(${
            margin.left + xScale.bandwidth() / 2 + xScaleLinear(data.mean)
          } 0)`
        )
    ...
    if (config.drawThreshold) {
      ...
      const thresholdGroup = svg
        ...
        .attr(
          "transform",
          `translate(${margin.left + xScale.bandwidth() / 2} 0)`
        )
      ...
      const path = d =>
        `${d3
          ...
          .x(d => xScale(d.x))
          .y(d => yScale(d.y))(d)}l0 ${
          innerHeight - yScale(data.content[data.content.length - 1].y) - 2
        }l${-innerWidth + xScale.bandwidth()} 0Z`
      ...
      const thresholdShape = thresholdGroup
        .append("path")
      ...
      const thresholdLineGroup = thresholdGroup
        .append("g")
      ...
      const dragStart = () => {
        ...
        gsap.to(".dpd-interactive-prompt-wrap", {
          opacity: 0,
          ease: "expo.out",
          duration: 0.8,
        })
        gsap.to(".dpd-interactive-results-wrap", {
          opacity: 1,
          ease: "expo.out",
          delay: 0.2,
          duration: 0.8,
        })
        gsap.to(".dpd-scroll-arrow", {
          opacity: 1,
          ease: "expo.out",
          delay: 0.2,
          duration: 0.8,
        })
        gsap.to(".dpd-scroll-arrow-label", {
          opacity: 1,
          ease: "expo.out",
          delay: 0.2,
          duration: 0.8,
    ...
    animations.forEach(section => {
      ...
      section.animations.forEach(animation => {
        ...
        if (animation.from) {
          ...
          timeline.add(
            gsap.to(`${target} ${animation.target}`, toObject),
            insertionTime
          )
        }
      })
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.threshold !== this.props.threshold) {
      ...
      d3.select(`.${className} .threshold-label-text`).text(
        `Threshold: ${d3.format("0.1f")(threshold)}`
      )

      gsap.to(`.${className} .threshold-line-g`, { x: newX, duration: 0.1 })
      gsap.to(`.${className} .threshold-mask-rect`, {
        scaleX: 1 - (threshold - 1) / 9,
        duration: 0.1,
      })
    }
  }
