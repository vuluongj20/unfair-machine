...
const introFunctions = {
  intro:
    "Run the model with the variable Sex, and then without. Take note of the positive prediction rate for male and female applicants in both cases.",
  introLabel: cta =>
    `${
    ...
    setTimeout(() => {
      ...
      this.stInstance = ScrollTrigger.create({
        ...
        animation: gsap
          ...
          .add(
            gsap.fromTo(
              ".gur-illustration-circle-path",
              { drawSVG: 0 },
              { drawSVG: `100%`, ease: "expo.out", duration: 0.6 }
            ),
            0
          )
          .add(
            gsap.fromTo(
              ".gur-illustration-arrow-line",
              { drawSVG: 0 },
              {
                drawSVG: `100%`,
                ease: "expo.out",
        ...
        >
          ...
          <div className="gur-illustration-inner-wrap">
            {illustration.variables.map((variable, ind) => {
              const arrowIndices = illustration.arrows.filter(
                el => el[0] === ind
              )
              return (
                <div key={ind} className="gur-variable-wrap">
                  ...
                  {wrapWidth &&
                    arrowIndices.map((arrow, ind) => {
                      const arrowWidth =
                        (wrapWidth / (illustration.variables.length - 1)) *
                          Math.abs(arrow[1] - arrow[0]) -
                        32
                      const flipped = arrow[1] - arrow[0] < 0
                      return (
                        <div
                          key={ind}
                          className={`gur-illustration-arrow-wrap ${
                            flipped ? "flipped" : ""
                          }`}
                        >
                          <svg
                          ...
                          >
                            <path
                              d={`M2 120C1 -32 ${arrowWidth} -32 ${
                                arrowWidth - 2
                              } 120`}
                              className="gur-illustration-arrow-line"
                            />
                            <path
                              d={`M${arrowWidth - 8} 114L${
                                arrowWidth - 2
                              } 120L${arrowWidth + 4} 114`}
