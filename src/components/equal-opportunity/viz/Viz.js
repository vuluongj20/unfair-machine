...
class Viz extends Component {
    ...
    animations.forEach(section => {
      ...
      section.animations.forEach(animation => {
        const toObject = Object.assign({
          ...
          stagger: animation.stagger ? (animation.microStagger ? 0.0002 : 0.02) : 0,
        ...
        if (animation.from) {
          timeline.add(gsap.fromTo(
            animation.targetOutside ? animation.target : `${target} ${animation.target}`, 
            animation.from, 
            toObject), insertionTime)
        } else {
  ...
  setThresholdHover(e) {
    ...
    const mouseX = e.clientX - vizBBox.left
    this.setState({
      thresholdHover: Math.max(Math.min(mouseX / vizBBox.width * 100, 100), 0)
    })
  }
  ...
  updateRates(threshold) {
    ...
    const negativeIndex = binarySearch(data.negative, threshold / 100, (element, needle) => element.x - needle)

    const truePositiveRate = ((data.positive.length + positiveIndex + 1) / data.positive.length).toFixed(2)
    const falsePositiveRate = ((data.negative.length + negativeIndex + 1) / data.negative.length).toFixed(2)
  ...
  componentDidMount() {
    const { data, distributions, className, animations, animationTriggers, animationOverrides, threshold } = this.props
    ...
    return (
      <div className={`eo-viz ${className} flex-centers spread`}>
        ...
        <div 
          ...
          onMouseLeave={e => options.showThreshold && this.toggleThresholdHover(false)}
        >
