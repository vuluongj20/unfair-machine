...
class Sandbox extends Component {
  ...
  trainModel(input, label) {
    ...
    model.fit(
      ...
      label,
      {
        ...
        callbacks: {
          onEpochEnd: (epoch, logs) => {
            ...
            gsap.to('#gur-sandbox .gud-progress-number', { 
            ...
            gsap.to('#gur-sandbox #gud-progress-line', { 
      ...
      const box = document.querySelector('#gur-sandbox .gud-box-status')
      if (box) {
        box.innerText = 'Done!'
      }
  ...
  async modelCallback(model) {
    ...
    this.setState({
      ...
      gsap.fromTo('.gud-attribute-wrap', 
        { opacity: 1 }, 
  ...
  retry() {
    this.setState({
      ...
      gsap.to('#gur-sandbox .gud-attribute-wrap', { 
        opacity: 1, 
        ease: 'expo.out', 
        duration: 0.6 
      })
