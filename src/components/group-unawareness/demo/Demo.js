...
const content = {
  intro: (direction) => `You\'re a machine learning engineer at a commercial bank. One day, your manager gives you a large dataset and asks you to build an algorithm that classifies potential clients into two income types: those who make more than 50K/year, and those who make less. You have access to the attributes ${direction}. Which of those attributes would you use?`,
...
class Demo extends Component {
  ...
  trainModel(input, label) {
    ...
    model.fit(
      ...
      label,
      {
        ...
        callbacks: {
          onEpochEnd: (epoch, logs) => {
            ...
            gsap.to('#gud .gud-progress-number', { 
      ...
      const box = document.querySelector('#gud .gud-box-status')
      if (box) {
        box.innerText = 'Done!'
      }
            ...
            <div className="gud-content-wrap">
              <div className="gud-intro-wrap absolute-center">
                <p className="gud-explanation">
                  {content.intro(screenWidth > 768 ? 'on the left' : 'on top')}
                </p>
                <p className="gud-explanation-label">
                  {content.introLabel(whatInput.ask() === 'mouse' ? 'click' : 'tap')}
                </p>
                <button
